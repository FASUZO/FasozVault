...
> ⚡ **声明：本项目前期主要由 [Cursor](https://cursor.sh) AI 编码助手协助生成与维护。后期转换为[Gemini](https://gemini.google.com/)进行编写、优化**

---

## 📖 目录

- [💼 FasozVault - 个人资产管理系统](#-fasozvault---个人资产管理系统)
  - [📖 目录](#-目录)
  - [项目亮点](#项目亮点)
  - [功能特性](#功能特性)
  - [技术栈](#技术栈)
...
# 2. 创建环境变量文件（首次部署）
$ cp env.example .env
$ nano .env  # 根据需要修改配置（可选）

# 3. 构建并启动服务
# docker-compose.yml 文件已包含在项目中，无需手动创建
...
**注意**：项目已包含 `docker-compose.yml` 配置文件，无需手动创建。首次运行时会自动构建镜像。

#### 使用 Docker 命令（不使用 docker-compose）

```bash
# 1. 构建镜像（必须先构建，因为镜像未上传到 Docker Hub）
$ docker build -t fasozvault:latest .

# 2. 创建环境变量文件（可选）
$ cp env.example .env

# 3. 运行容器（数据持久化到宿主机 ./data）
$ docker run -d \
  --name fasozvault \
  -p 3000:3000 \
  -v $(pwd)/data:/app/data \
...
#### 从 Docker Hub 拉取镜像（如果已上传）

如果镜像已上传到 Docker Hub，可以使用：

```bash
# 拉取镜像（需要先上传到 Docker Hub）
...
$ docker run -d \
  ...
  -p 3000:3000 \
  -v $(pwd)/data:/app/data \
  your-username/fasozvault:latest
```
...
# 2. 构建镜像
$ docker build -t your-username/fasozvault:latest .

# 3. 推送镜像
$ docker push your-username/fasozvault:latest
