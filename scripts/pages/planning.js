/**
 * FasozVault - 规划/待办清单页面脚本
 * 功能：待办任务添加、任务列表展示、任务完成/删除、任务数据持久化
 * 
 * 使用页面：pages/planning.html
 * 
 * 调试提示：
 * - localStorage key "todoTasks" 保存任务 JSON 数组
 * - 在控制台执行 `tasks` 查看当前内存任务
 * - 修改后调用 `saveTasks()` 手动保存
 * - 调用 `loadTasks()` 可重新从 localStorage 读取
 */
